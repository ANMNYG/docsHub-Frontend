...
export function DocumentPage() {
  ...
  const { items, clear } = useDocumentHistory();
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <h1 className="font-display text-xl font-semibold text-ink truncate">{fileName}</h1>
          </div>
          {markdown && (
            <div className="flex shrink-0 gap-2">
              <button
                onClick={handleCopy}
                className="rounded-lg border border-line bg-surface px-3 py-1.5 text-xs font-medium text-ink hover:border-accent transition-colors"
              >
                {copied ? "복사됨 ✓" : "복사"}
              </button>
              <button
          ...
          {isError && (
            <div className="py-8 text-center">
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => refetch()}
                  className="rounded-lg border border-line px-3 py-1.5 text-xs font-medium hover:border-accent transition-colors"
                >
                  다시 시도
                </button>
                <Link
